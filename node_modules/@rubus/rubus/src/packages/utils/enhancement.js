...
function toggleInputGroupFocus(event) {
  ...
  if (classList) {
    if (classList.contains("spectrum-InputGroup-input") || classList.contains("spectrum-FieldButton")) {
  ...
  var closestElement = event.target.closest && event.target.closest(closestSelector);
  if (closestElement) {
  ...
  function onMouseMove(e, sliderHandle) {
    ...
    if (buffers.length) {
      if (percent >= bufferedAmount) {
        ...
        var handlePosition =
          toggleRTL(handle.offsetLeft, handle.parentElement.offsetWidth - handle.offsetLeft) + handle.offsetWidth / 2;
